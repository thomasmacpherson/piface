...
boardButtonsX = [15.0, 40.0, 65.0, 90.0]
boardButtonsY = [160.0, 160.0, 160.0, 160.0]
boardButtonStatus = [False,False,False,False]

inputArray =[False,False,False,False,False,False,False,False]
outputArray =[False,False,False,False,False,False,False]
...
#        cr.set_source_rgb( 1,0,0 )
	...
	def led_on(status):
		...
		cr.arc (ledsX[status], ledsY[status], 8, 0, 2*pi);
		cr.fill( )
		cr.restore ( )

		cr.save ( )
		cr.set_source_rgb( 1,0,0 )
		cr.arc (ledsX[status], ledsY[status], 6, 0, 2*pi);
		cr.fill( )
		cr.restore ( )
...
def physically_change_pin(pin,status):
	global inputPins
	pin = pinTranslation(pin)
